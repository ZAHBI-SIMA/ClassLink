...
export async function getBulletinList(
  ...
  const rows: any[] = await db.$queryRaw`
    SELECT
      ...
      ROUND(
        SUM(
          (SUM(g.value * g.coefficient) / NULLIF(SUM(g.coefficient), 0))
          * COALESCE(ls.coefficient, 1)
        )::numeric / NULLIF(SUM(COALESCE(ls.coefficient, 1)), 0),
        2
      )                                                 AS average,
      RANK() OVER (
        ...
        ORDER BY
          SUM(
            (SUM(g.value * g.coefficient) / NULLIF(SUM(g.coefficient), 0))
            * COALESCE(ls.coefficient, 1)
          ) / NULLIF(SUM(COALESCE(ls.coefficient, 1)), 0) DESC NULLS LAST
      )                                                 AS rank,
      cs.decision
    FROM students s
    JOIN users u ON u.id = s.user_id
    ...
    GROUP BY s.id, u.first_name, u.last_name, c.name, e.class_id, cs.decision
    ORDER BY rank ASC, u.last_name, u.first_name
  `
  return rows
}

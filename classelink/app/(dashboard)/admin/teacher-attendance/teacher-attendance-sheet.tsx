...
import { saveTeacherAttendance } from '@/actions/admin'
...
export function TeacherAttendanceSheet({ teachers: initial, date }: Props) {
  const [statuses, setStatuses] = useState<Record<string, AttendanceStatus>>(
    Object.fromEntries(initial.map(t => [t.teacher_id, t.status]))
  )
  ...
  function buildFormData() {
    ...
    for (const t of initial) {
      fd.append('teacher_id', t.teacher_id)
      fd.set(`status_${t.teacher_id}`, statuses[t.teacher_id])
    }
    return fd
  }
  ...
  function markAll(status: AttendanceStatus) {
    setStatuses(Object.fromEntries(initial.map(t => [t.teacher_id, status])))
  }
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {initial.map(teacher => {
              const current = statuses[teacher.teacher_id]
              return (
                    ...
                    <div className="flex justify-center gap-2 flex-wrap">
                      {(Object.keys(STATUS_CONFIG) as AttendanceStatus[]).map(s => (
                        <button
                          key={s}
                          type="button"
                          onClick={() => setStatuses(prev => ({ ...prev, [teacher.teacher_id]: s }))}

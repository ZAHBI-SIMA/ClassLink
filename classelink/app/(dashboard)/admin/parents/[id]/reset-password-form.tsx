...
import { resetParentPassword } from '@/actions/admin'
import { useState } from 'react'

export function ResetParentPasswordForm({ userId }: { userId: string }) {
  const [state, action, isPending] = useActionState(resetParentPassword, null)
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5">
      ...
      {state?.success && (state as any).data?.tempPassword && (
        <div className="mb-4 p-3 rounded-lg bg-green-50 border border-green-200">
          ...
          <p className="text-xs text-green-600 mt-1.5">Communiquez ce mot de passe au parent, il ne sera plus affiché.</p>
        </div>
      )}
